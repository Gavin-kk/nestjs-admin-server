            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        user = _a.sent();
                        if (!user) {
                            throw new common_1.HttpException({ status: 400, msg: '用户不存在' }, 400);
                        }
                        // 验证密码是否正确
                        if (!bcryptjs_1.compareSync(password, user.password)) {
                            throw new common_1.HttpException({ status: 400, msg: '用户名或密码错误' }, 400);
                        }
...
}(passport_1.PassportStrategy(passport_local_1.Strategy, 'local')));
